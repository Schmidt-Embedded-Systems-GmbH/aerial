...
let print_event p param =
  match param with
    | Zero -> print_string (Printf.sprintf "%s ()\n" p )
...
let relations_start asize msize = 
  ...
  let print_entry_a = print_entry1 "r" in  
  let print_entry_m = print_entry1 "r" in
...
let gen_log_P1 () = 
  ...
  for ts = !first_ts to !first_ts + !time_span - 1 do
    ...
    for j = 1 to nbe do      
      ...
      let m = get_mgr (ts_app mod tw) msize a in
      Queue.add ("q", Zero) ev_array.(ts_app mod etw);
      Queue.add ("p", Zero) ev_array.(ts mod etw);
    done;
	...
	begin
	  ...
	  Queue.add ("r", Zero) ev_array.(ts' mod etw);
	end
    done;
    let nbe = nb_of_events (mgr_rate / 2) var in
    for i = 1 to nbe do
...
let gen_trans_events (tsize, csize, esize, trans_rate, gs_trans_rate, ts_start, var) = 
  ...
  for ts = ts_start to ts_start + !time_span do
	...
	pqueue := PrioQueue.insert !pqueue ts ("p", Zero);
	if !policy = "P3" then
	  pqueue := PrioQueue.insert !pqueue ts_a ("r", Zero)
	else
	  pqueue := PrioQueue.insert !pqueue ts_r ("q", Zero)
      else
	pqueue := PrioQueue.insert !pqueue ts ("p", Zero)	  
    done;

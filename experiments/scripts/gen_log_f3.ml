...
let random_element l  total= 
	...
	let print_event p param =
	  match param with
	    | One x -> print_string (Printf.sprintf "P%d ()\n" (random_element (get_list 0 1 2 1 1 1 ) 3))
	    | Two (x,y) -> (let r1 = (random_element (get_list 0 1 2 1 1 1 ) 3) in
							   let r2 = (random_element (get_list 0 1 2 1 1 1 ) 3) in 
							   let r3 = (random_element (get_list 0 1 2 1 1 1 ) 3) in 
							   if (r1 = r2 & r2 = r3) then print_string (Printf.sprintf "P%d ()\n" r1)
							   else if r1 = r2 then print_string (Printf.sprintf "P%d () P%d ()\n" r1 r3) 
							   else if r1 = r3 then print_string (Printf.sprintf "P%d () P%d ()\n" r1 r2) 
							   else if r2 = r3 then print_string (Printf.sprintf "P%d () P%d ()\n" r1 r3) 
						   	   else print_string (Printf.sprintf "P%d () P%d () P%d ()\n" r1 r2 r3) )
...
let gen_log_P1 () = 
  ...
  for ts = !first_ts to !first_ts + !time_span - 1 do
	...
	begin
	  ...
	  Queue.add  ((*"mgr_S"*)"", One (m)) ev_array.(ts' mod etw);
	end
    done;
    let nbe = nb_of_events (mgr_rate / 2) var in
    for i = 1 to nbe do
	...
	begin
	  ...
	  Queue.add  ((*"mgr_F"*)"", One (m)) ev_array.(ts' mod etw);
	end
    done;
